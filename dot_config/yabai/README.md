...
## Preparing for / recovering from macOS update

- Before update, restore SIP to normal settings. I believe not doing this is causing the issue where macOS is corrupted after update.
- After update, redo SIP config as per Yabai docs.
- Quit all open apps and reopen.
- If Yabai is still not able to take full control, reload the scripting addition: `sudo yabai --load-sa`.
- Quit all open apps and reopen.
