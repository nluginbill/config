function nid --wraps=nb --description 'Helper for `nb` ideas/dev.md folder'
    ...
    if test -n "$filename"
        ...
        set todo_item (nb tasks ideas/dev.md $filename)
        if test -n "$todo_item"
            if string match -q -r "ideas/\d.*\ .*$filename.*\ .*\[.*\ .*\]" $todo_item
                nb do ideas/dev.md $filename
                return 0
            else
                nb undo ideas/dev.md $filename
                return 0
            end
        end
    else if test -z "$path" -a -n "$content"
        # If only content is provided
        nb list ideas/dev.md && nb edit ideas/dev.md --content "$content" || nb add ideas/dev.md --content "$content"
